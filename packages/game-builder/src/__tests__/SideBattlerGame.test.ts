...
function skipToPlayerTurn(game: SideBattlerGame, playerId: string): void {
  for (let i = 0; i < 20; i++) {
    ...
    if (!entry) return;
    const char = data.party.find((c: any) => c.id === entry.id && c.alive);
...
describe('SideBattlerGame', () => {
  ...
  describe('initialization', () => {
    it('sets phase to playing and battlePhase to prep', () => {
      ...
      expect((state.data as any).battlePhase).toBe('prep');
    });
    ...
    it('creates a party of 4 characters: warrior, mage, archer, healer', () => {
      ...
      expect(data.party.map((c: any) => c.classType)).toEqual([
        'warrior',
        'mage',
        'archer',
        'healer',
  ...
  describe('turn order', () => {
    it('is sorted by SPD with highest first', () => {
      ...
      act(game, 'player-1', 'start_wave');
      const data = game.getState().data as any;
      const turnOrder = data.turnOrder as any[];
      const speeds: number[] = turnOrder.map((entry: any) => {
        if (entry.type === 'party') {
          return data.party.find((c: any) => c.id === entry.id)?.stats.spd ?? 0;
        }
        return data.enemies.find((e: any) => e.id === entry.id)?.stats.spd ?? 0;
      });
    ...
    it('includes both party and enemy entries', () => {
      ...
      act(game, 'player-1', 'start_wave');
      const data = game.getState().data as any;
      const types = data.turnOrder.map((e: any) => e.type);
  ...
  describe('basic attack', () => {
    it('deals physical damage equal to ATK minus DEF/2 with minimum 1', () => {
      ...
      const warrior = dataBefore.party.find((c: any) => c.classType === 'warrior');
      const aliveEnemies = dataBefore.enemies.filter((e: any) => e.alive);
      ...
      const dataAfter = game.getState().data as any;
    ...
    it('applies back row melee penalty of 0.85x for ranged characters', () => {
      ...
      const dataBefore = game.getState().data as any;
      const archer = dataBefore.party.find((c: any) => c.classType === 'archer');
      const aliveEnemies = dataBefore.enemies.filter((e: any) => e.alive);
      const enemyHpBefore = aliveEnemies[0].stats.hp;
      const enemyDef = aliveEnemies[0].stats.def;
    ...
    it('cannot attack on wrong player turn in co-op', () => {
      ...
      const data = game.getState().data as any;
      const entry = data.turnOrder[data.currentTurnIndex];
      const char = data.party.find((c: any) => c.id === entry?.id);
      if (!char) return;
    ...
    it('kills enemy when HP reaches 0 and marks alive as false', () => {
      ...
      const data = game.getState().data as any;
      const aliveEnemy = data.enemies.find((e: any) => e.alive);
      aliveEnemy.stats.hp = 1;

      act(game, 'player-1', 'attack');

      const dataAfter = game.getState().data as any;
      const target = dataAfter.enemies.find((e: any) => e.id === aliveEnemy.id);
  ...
  describe('defend', () => {
    ...
    it('restores 3 MP when defending', () => {
      ...
      const data = game.getState().data as any;
      const warrior = data.party.find((c: any) => c.classType === 'warrior');
      warrior.stats.mp = 5;

      act(game, 'player-1', 'defend');

      const dataAfter = game.getState().data as any;
      const warriorAfter = dataAfter.party.find((c: any) => c.classType === 'warrior');
  ...
  describe('skills', () => {
    ...
    it('mage Fireball deals 2x magical damage using MATK minus MDEF/3', () => {
      ...
      const totalEnemyHpBefore = dataBefore.enemies.reduce(
        (s: number, e: any) => s + Math.max(0, e.stats.hp),
        0,
      );
      ...
      const dataAfter = game.getState().data as any;
      const totalEnemyHpAfter = dataAfter.enemies.reduce(
        (s: number, e: any) => s + Math.max(0, e.stats.hp),
        0,
      );
    ...
    it('archer Snipe deals 2.5x ATK damage and ignores enemy DEF', () => {
      ...
      const dataAfter = game.getState().data as any;
      const enemyAfter = dataAfter.enemies.find((e: any) => e.id === targetEnemy.id);
    ...
    it('AoE Blizzard damages all living enemies', () => {
      ...
      const dataBefore = game.getState().data as any;
      const enemyHpsBefore = dataBefore.enemies.map((e: any) => e.stats.hp);
    ...
    it('Poison Shot applies poison status effect to the target', () => {
      ...
      const data = game.getState().data as any;
      const target = data.enemies.find((e: any) => e.alive);
      if (!target) return;
      const poison = target.statusEffects.find((e: any) => e.type === 'poison');
  ...
  describe('status effects', () => {
    it('poison deals damage at round end', () => {
      ...
      for (let i = 0; i < 30; i++) {
        const d = game.getState().data as any;
        if (d.battlePhase !== 'combat') break;
        const entry = d.turnOrder[d.currentTurnIndex];
        if (!entry) break;
        const char = d.party.find((c: any) => c.id === entry.id && c.alive);
        if (char) {
          act(game, 'player-1', 'defend');
        } else {
    ...
    it('taunt forces enemies to attack the taunter', () => {
      ...
      const mage = data.party.find((c: any) => c.classType === 'mage');
      mage.stats.hp = 5;

      act(game, 'player-1', 'auto_tick');

      const dataAfter = game.getState().data as any;
      const mageAfter = dataAfter.party.find((c: any) => c.classType === 'mage');
    ...
    it('def_up adds defense buff with correct value and duration', () => {
      ...
      const data = game.getState().data as any;
      const warrior = data.party.find((c: any) => c.classType === 'warrior');
      const defUp = warrior.statusEffects.find((e: any) => e.type === 'def_up');
    ...
    it('status effects expire after their duration runs out', () => {
      ...
      const dataAfter = game.getState().data as any;
      const warriorAfter = dataAfter.party.find((c: any) => c.classType === 'warrior');
      const taunt = warriorAfter?.statusEffects?.find((e: any) => e.type === 'taunt');
  ...
  describe('enemy AI (auto_tick)', () => {
    ...
    it('taunted enemies attack the taunter instead of the weakest', () => {
      ...
      const healer = data.party.find((c: any) => c.classType === 'healer');
      healer.stats.hp = 1;

      act(game, 'player-1', 'auto_tick');

      const dataAfter = game.getState().data as any;
      const healerAfter = dataAfter.party.find((c: any) => c.classType === 'healer');
    ...
    it('boss attacks all party members at once', () => {
      ...
      const data = game.getState().data as any;
      expect(data.enemies.some((e: any) => e.isBoss)).toBe(true);

      const hpsBefore = data.party.filter((c: any) => c.alive).map((c: any) => c.stats.hp);
      ...
      const dataAfter = game.getState().data as any;
      const hpsAfter = dataAfter.party.map((c: any) => Math.max(0, c.stats.hp));
  ...
  describe('fog of war', () => {
    it('getStateForPlayer hides enemy status effect source IDs', () => {
      ...
      const pData = playerState.data as any;
    ...
    it('getStateForPlayer limits turn order to only the current entry', () => {
      ...
      const fullData = game.getState().data as any;
  ...
  describe('scoring', () => {
    ...
    it('survival bonus awards 100 per alive character', () => {
      ...
      const data = game.getState().data as any;
      const aliveCount = data.party.filter((c: any) => c.alive).length;
